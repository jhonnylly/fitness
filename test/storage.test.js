...
(async () => {
  ...
  ok(app.sesionImpuesta({sesion:3}, rutAviso, {}) === 3,
     'un encargo con sesión manda a esa sesión');
  ok(app.sesionImpuesta({sesion:3}, rutAviso, {}) === 3,
     '🔴 el cambio de día vive en sesionCoach, campo APARTE del mensaje (falló dos veces el 09/09)');
  ok(app.sesionImpuesta({}, rutAviso, {}) === null,
     'un encargo sin sesión no cambia nada');
  ok(app.sesionImpuesta({sesion:3}, rutAviso, {3:{date:'9/9/2026'}}) === null,
     'cuando ya ha hecho esa sesión, el encargo se acaba y vuelve a mandar la app');
  ok(app.sesionImpuesta({sesion:3}, rutAviso, {1:{date:'9/9/2026'}}) === 3,
     'haber hecho OTRA sesión no lo cancela');
  ok(app.sesionImpuesta({sesion:99}, rutAviso, {}) === null,
     'si apunta a una sesión que ya no existe, no deja Inicio sin botón');
  ok(app.sesionImpuesta(null, rutAviso, {}) === null, 'sin encargo, null');
  ok(app.sesionImpuesta({sesion:1}, null, {}) === null, 'sin rutina activa, null');
